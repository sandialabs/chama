...
class UtilsTests(unittest.TestCase):
    ...
    def test_df_columns_required(self):
        ...
        cu._df_columns_required('sample_data', df, {'Col1': object, 'Col2': np.float64, 'Col3': np.int64})

        # test incorrect type - object
        with self.assertRaises(TypeError):
            cu._df_columns_required('sample_data', df, {'Col1': str, 'Col2': np.float64, 'Col3': np.int64})

        # test incorrect type - float
        with self.assertRaises(TypeError):
            cu._df_columns_required('sample_data', df, {'Col1': object, 'Col2': str, 'Col3': np.int64})

        # test incorrect type - int
        with self.assertRaises(TypeError):
            cu._df_columns_required('sample_data', df, {'Col1': object, 'Col2': np.float64, 'Col3': np.float64})

        # test passing multiple types
        cu._df_columns_required('sample_data', df, {'Col1': object, 'Col2': [np.int64, np.float64], 'Col3': [np.int64, np.float64]})

        # test passing multiple types with incorrect types
        with self.assertRaises(TypeError):
            cu._df_columns_required('sample_data', df, {'Col1': object, 'Col2': [np.int64, np.bool8], 'Col3': [np.int64, np.float64]})

    def test_df_columns_exist(self):
        df = pd.DataFrame.from_dict(sample_data)

        # test all good - raises exception (and fails test if not)
        self.assertTrue(cu._df_columns_exist(df, {'Col1': object, 'Col2': np.float64, 'Col3': np.int64})) 

        # test incorrect type - object
        self.assertFalse(cu._df_columns_exist(df, {'Col1': str, 'Col2': np.float64, 'Col3': np.int64}))

        # test incorrect type - float
        self.assertFalse(cu._df_columns_exist(df, {'Col1': object, 'Col2': str, 'Col3': np.int64}))
        
        # test incorrect type - int
        self.assertFalse(cu._df_columns_exist(df, {'Col1': object, 'Col2': np.float64, 'Col3': np.float64}))

        # test passing multiple types
        self.assertTrue(cu._df_columns_exist(df, {'Col1': object, 'Col2': [np.int64, np.float64], 'Col3': [np.int64, np.float64]}))

        # test passing multiple types with incorrect types
        self.assertFalse(cu._df_columns_exist(df, {'Col1': object, 'Col2': [np.int64, np.bool8], 'Col3': [np.int64, np.float64]}))
    ...
    def test_df_columns_nans_not_allowed(self):
        ...
        # test for NAN in col2 or col3 (True)
        with self.assertRaises(TypeError):
            cu._df_columns_nans_not_allowed('sample_data2', df, ['Col2', 'Col3'])
        ...
        # test for exception if column name does not exist
        with self.assertRaises(TypeError):
            cu._df_columns_nans_not_allowed('sample_data2', df, ['Col1', 'ColDoesNotExist'])
        with self.assertRaises(TypeError):
            cu._df_columns_nans_not_allowed('sample_data2', df, 'ColDoesNotExist')
