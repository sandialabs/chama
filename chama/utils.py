...
def df_columns_exist(df, col_type_dict):
    """ Internal function that returns False if a column in the dataframe
...
def df_nans_not_allowed(df_name, df):
    """ Internal function that raises an exception if a user passed a
    dataframe with NANs when not allowed.
    """
...
def df_columns_nans_not_allowed(df_name, df, col_list):
    """ Internal function that raises an exception if user passed in a
    dataframe with NANs in a particular column (where col names are in
    col_list)
    """
