...
def df_columns_required(df_name, df, col_type_dict):
    """ Internal function that raises an exception if dataframe does not contain the expected columns with expected types
    """
...
def df_columns_exist(df, col_type_dict):
    """ Internal function that returns False if a column in the dataframe does not exist or is not of the expected type
    """
...
def df_nans_not_allowed(df_name, df):
    """ Internal function that raises an exception if a user passed a dataframe with NANs when not allowed.
    """
...
def df_columns_nans_not_allowed(df_name, df, col_list):
    """ Internal function that raises an exception if user passed in a dataframe with NANs in a particular column (where col names are in col_list)
    """
...
def df_columns_has_nans(df, col_list):
    ...
    for name in new_col_list:
        if name not in df.keys():
            raise TypeError('Test for NAN in column %s that does not exist in dataframe' % (name))
